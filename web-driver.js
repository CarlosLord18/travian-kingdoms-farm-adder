...
const CHROME_OPTIONS = {
	'args': [
		...
		"--test-type", "--start-maximized", "user-data-dir=C:\\Users\\PanTau_acc\\AppData\\Local\\Google\\Chrome\\User Data"
	],
	'prefs': {
...
module.exports = class WebDriver {
	...
	async getElementValue(xPath) {
		let res = ""
		let state = false
		await this.driver.findElement(By.xpath(xPath)).getAttribute('value').then((text) => {
			res = text
			state = true
		}).catch(e => { res = -1; state = true; })
		require('deasync').loopWhile(() => { return !state })
		return res
	}
	...
	async waitForElement(xPath) {
		await this.driver.wait(until.elementIsVisible(this.driver.findElement(By.xpath(xPath))), 100000, "Wait for " + xPath)
	}

	async clickOnElement(xPath) {
		await this.driver.findElement(By.xpath(xPath)).click()
	}

	async clickOnWebElement(webDriverElement) {
